# iwo Mail Client

🇬🇧 [English version](README_EN.md)

Почтовый клиент для Android с поддержкой Microsoft Exchange Server 2007+ (ActiveSync), IMAP и POP3.

**Версия:** 1.0.9  
**Разработчик:** DedovMosol  
**Telegram:** [@i_wantout](https://t.me/i_wantout)  
**Email:** andreyid@outlook.com

## 🌟 Особенности

- 📧 **Exchange ActiveSync** — полная поддержка EAS 2.5-14.1
- 📬 **IMAP/POP3** — работа с любыми почтовыми серверами (beta)
- 🔒 **Совместимость с Exchange 2007** — поддержка TLS 1.0/1.1 через Conscrypt
- 🌍 **Два языка** — 🇷🇺 Русский / 🇬🇧 English
- 🎨 **Material Design 3** — современный интерфейс с цветовыми темами
- 🔔 **Push-уведомления** — мгновенные уведомления о новых письмах (Direct Push)

## 📱 Возможности

| Функция | Статус |
|---------|--------|
| Синхронизация папок и писем | ✅ |
| Отправка писем с вложениями | ✅ |
| Скачивание вложений | ✅ |
| Поиск по письмам | ✅ |
| Фильтры (непрочитанные, помеченные, с вложениями, по дате) | ✅ |
| Избранные письма | ✅ |
| Перемещение/удаление/восстановление писем | ✅ |
| Отложенное удаление с отменой | ✅ |
| Автоочистка корзины | ✅ |
| Создание/удаление/переименование папок | ✅ |
| Контакты с импортом/экспортом | ✅ |
| Автодополнение email | ✅ |
| Несколько аккаунтов | ✅ |
| Push-уведомления (Direct Push) | ✅ |
| Фоновая синхронизация | ✅ |
| Тёмная тема | ✅ |
| Цветовые темы (7 цветов) | ✅ |
| Подпись для аккаунта | ✅ |
| Запрос отчёта о прочтении/доставке (MDN/DSN) | ✅ |

## 🆕 Что нового в v1.0.9

- **Контакты** — полноценная система управления контактами с импортом/экспортом
- **Автодополнение адресов** — подсказки из контактов, истории и GAL при вводе email
- **Стилизованные диалоги** — единый градиентный дизайн для всех диалогов
- **Управление сертификатом** — просмотр, экспорт, замена и удаление сертификата сервера
- **Фоновая синхронизация** — папки синхронизируются в фоне, данные показываются мгновенно

📋 Полная история изменений: [CHANGELOG_RU.md](CHANGELOG_RU.md)

## 📋 Требования

- Android 8.0+ (API 26)
- Target SDK: 35 (Android 15)

## 🖥️ Поддерживаемые серверы

| Сервер | Статус |
|--------|--------|
| Exchange 2007 SP1+ | ✅ Полная поддержка |
| Exchange 2010-2019 | ✅ Работает |
| Office 365 | ⚠️ Требует OAuth |
| IMAP/POP3 серверы | ⚠️ Beta |

## 🔧 Сборка

```bash
./gradlew assembleDebug    # Debug
./gradlew assembleRelease  # Release
```

## 📖 Документация

- [История изменений](CHANGELOG_RU.md)
- [Политика конфиденциальности](PRIVACY_POLICY.md)

## 🏗️ Архитектура

```
UI Layer (Compose)
    ↓
Repository Layer (AccountRepository, MailRepository)
    ↓
Protocol Clients (EasClient, ImapClient, Pop3Client)
    ↓
Background Services (PushService, SyncWorker)
```

## 📄 Лицензия

MIT License

---

© 2025 DedovMosol
